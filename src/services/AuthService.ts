...
class AuthService {
    constructor(private users: typeof userService, private tokenRepo: Repository<RefreshToken>) { }
    ...
    async login(credentials: UserLoginBody): Promise<{ user: User, token: string, refreshToken: string }> {
    ...
    async getTokenFromDB(token: string): Promise<RefreshToken>{
        const refreshToken = await this.tokenRepo.findOne({where: {
            refreshToken: token
        }})
        if(!refreshToken){
            throw new UnauthorizedError("Your token is not valid")
